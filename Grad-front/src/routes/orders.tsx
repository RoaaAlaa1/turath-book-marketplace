...
function Orders() {
    ...
    const [bookCatalog, setBookCatalog] = useState<
        Record<string, { title: string; author: string; price: number }>
    >({});
    ...
    useEffect(() => {
        ...
        apiFetch<any[]>(`/api/Orders/${userId}`)
            .then((data) =>
                setApiOrders(
                    data.map((o: any) => {
                        const lines = (o.items ?? []).map((line: any) => {
                            const productId = String(line.productId ?? "");

                            const book = bookCatalog[productId] ?? {
                                title: line.title ?? `Book #${productId}`,
                                author: line.author ?? "",
                            ...
                            return {
                                bookId: productId,
                                title: line.title ?? book.title,
                                author: line.author ?? book.author,
                                price: Number(line.unitPrice ?? book.price ?? 0),
                                    ...
                                    {timeline.map((t, i) => (
                                        ...
                                        >
                                            <div className="flex flex-col items-center gap-1">
                                                ...
                                                <span
                                                    className={`text-[0.65rem] ${i <= stage
                                                            ? "text-foreground"
                                            ...
                                            {i < timeline.length - 1 && (
                                                <span
                                                    className={`h-px flex-1 ${i < stage ? "bg-primary" : "bg-border"
                                                        }`}
                                                />
                                            )}
                                        </li>
