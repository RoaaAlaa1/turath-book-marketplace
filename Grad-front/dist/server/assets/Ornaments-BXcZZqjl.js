...
function TurathProvider({ children }) {
	...
	const value = useMemo(() => {
		...
		let matchedUser = authUserId ? users.find((u) => u.id === authUserId) ?? users.find((u) => storedEmail && u.email.toLowerCase() === storedEmail.toLowerCase()) : null;
		if (!matchedUser && (authUserId || storedEmail)) try {
			const cached = (authUserId ? localStorage.getItem(`turath-profile-${authUserId}`) : null) || (storedEmail ? localStorage.getItem(`turath-profile-${storedEmail.toLowerCase()}`) : null);
			if (cached) matchedUser = JSON.parse(cached);
		} catch {}
		const activeUser = authUserId ? matchedUser ?? {
			id: authUserId,
...
//#endregion
export { cn as a, avgRating as c, useTurath as d, apiFetch as f, Button as i, egp as l, LeafSprig as n, TAX_RATE as o, currentUserId as p, Badge as r, TurathProvider as s, BranchDivider as t, roleLabels as u };
