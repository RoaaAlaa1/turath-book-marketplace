...
var STORAGE_KEY = "turath-state-v1";
var defaults = {
	role: "customer",
	authUserId: null,
	books: initialBooks,
...
function TurathProvider({ children }) {
	...
	const value = useMemo(() => {
		...
		const activeUser = users.find((u) => u.id === authUserId) ?? users.find((u) => u.id === roleUserId[role]) ?? users[0] ?? initialUsers[0];
		...
		return {
			...
			signIn: (email) => {
				...
				patch({
					authUserId: user.id,
					role: user.sellerState === "pending" ? "pendingSeller" : user.role
				});
				return true;
			},
			...
			bookById,
			sellerName: (id) => users.find((u) => u.id === id)?.name ?? "Unknown seller",
			...
			removeFromCart: (bookId) => patch({ cart: cart.filter((c) => c.bookId !== bookId) }),
			clearCart: () => patch({ cart: [] }),
			toggleWishlist: (bookId) => patch({ wishlist: wishlist.includes(bookId) ? wishlist.filter((w) => w !== bookId) : [...wishlist, bookId] }),
			placeOrder: (address) => {
			...
			cancelOrder: (orderId) => patch({ orders: orders.map((o) => o.id === orderId && o.status === "Pending" ? {
				...o,
				status: "Cancelled"
			} : o) }),
...
//#endregion
export { TAX_RATE as a, egp as c, cn as i, roleLabels as l, LeafSprig as n, TurathProvider as o, Badge as r, avgRating as s, BranchDivider as t, useTurath as u };
