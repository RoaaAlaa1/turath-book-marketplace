import { a as cn, f as roleLabels, i as Button, o as TAX_RATE, p as useTurath, r as Badge, s as TurathProvider, t as BranchDivider, u as egp } from "./Ornaments-JhP4ydzQ.js";
...
import { t as Route$6 } from "./orders-CtTX1Wsa.js";
...
var styles_default = "/assets/styles-BB1fQpfq.css";
//#endregion
...
function AuthSheet({ open, onOpenChange }) {
	...
	const register = async () => {
		...
		try {
			...
			if (!response.ok) {
				const errorMessages = Array.isArray(data?.errors) ? data.errors.map((item) => item?.description ?? item).join(". ") : typeof data?.errors === "object" ? Object.values(data.errors).flat().join(". ") : data?.message || data?.title || (typeof data === "string" && data ? data : "Registration failed.");
	...
	const submitSignIn = async () => {
		...
		try {
			...
			if (!response.ok) {
				const errorMsg = data?.message || data?.title || (typeof data === "string" && data ? data : "Invalid email or password.");
				setError(errorMsg);
				return;
			}
			if (data?.token) localStorage.setItem("token", data.token);
			const isAdminEmail = trimmedEmail.toLowerCase().startsWith("admin@turath.");
			data?.firstName && data?.lastName ? `${data.firstName} ${data.lastName}`.trim() : data?.username || isAdminEmail || trimmedEmail.split("@")[0];
			signIn(trimmedEmail);
			toast.success(remember ? "Signed in and remembered on this device" : "Signed in");
			onOpenChange(false);
		} catch (err) {
			if (signIn(trimmedEmail)) {
				toast.success(remember ? "Signed in and remembered on this device" : "Signed in");
				onOpenChange(false);
				return;
			}
...
function SignUp(props) {
	...
	if (props.step === 1) return /* @__PURE__ */ jsxs("div", {
		...
		children: [
			...
			}), emailIssueText && /* @__PURE__ */ jsx("div", {
				className: "mt-1",
				children: /* @__PURE__ */ jsx(ErrorText, { text: emailIssueText })
			})] }),
			/* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsx(Field, {
				label: "Password",
				type: "password",
			...
			}), passwordIssueText && /* @__PURE__ */ jsx("div", {
				className: "mt-1",
				children: /* @__PURE__ */ jsx(ErrorText, { text: passwordIssueText })
			})] }),
			/* @__PURE__ */ jsxs("div", {
				className: "space-y-1.5",
	...
	return /* @__PURE__ */ jsxs("div", {
		...
		children: [
			...
			/* @__PURE__ */ jsx(Button, {
				className: "w-full",
				onClick: props.onDone ?? props.onSignin,
				children: "Done"
			})
		]
	});
...
function Field({ label, value, onChange, type = "text", placeholder, autoComplete, onKeyDown }) {
	return /* @__PURE__ */ jsxs("div", {
		className: "space-y-1.5",
		children: [/* @__PURE__ */ jsx(Label, { children: label }), /* @__PURE__ */ jsx(Input, {
			type,
...
function AuthBadge({ onClick }) {
	...
	const effectiveRole = activeUser?.role === "admin" || role === "admin" ? "admin" : activeUser?.role === "seller" || activeUser?.sellerState === "approved" || role === "seller" ? "seller" : "customer";
	...
	return /* @__PURE__ */ jsxs(Button, {
		...
		children: [
			/* @__PURE__ */ jsx("span", {
				className: "flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-accent font-serif text-sm text-accent-foreground",
				children: isAuthenticated && activeUser ? activeUser.name.charAt(0).toUpperCase() : "J"
			}),
			/* @__PURE__ */ jsxs("div", {
				className: "flex flex-col text-left leading-tight min-w-0",
				children: [/* @__PURE__ */ jsx("span", {
					className: "truncate text-xs font-medium",
					children: isAuthenticated && activeUser ? activeUser.name : "Join us"
				}), isAuthenticated && /* @__PURE__ */ jsx("span", {
					className: "text-[10px] text-muted-foreground",
					children: isAdmin ? "Admin Steward" : isSeller ? "Seller" : "Reader"
				})]
			}),
			isAuthenticated && isAdmin && /* @__PURE__ */ jsx(Badge, {
				variant: "secondary",
...
function Layout({ children }) {
	...
	const safeRole = role === "admin" || activeUser?.role === "admin" || Boolean(activeUser?.email?.toLowerCase().startsWith("admin@turath.")) ? "admin" : role === "seller" || activeUser?.role === "seller" || activeUser?.sellerState === "approved" ? "seller" : activeUser?.sellerState === "pending" ? "customer" : role ?? "customer";
...
var $$splitComponentImporter$4 = () => import("./routes-Bm9LnCTv.js");
...
var $$splitComponentImporter$3 = () => import("./account-DKaXFpIs.js");
...
var $$splitComponentImporter$2 = () => import("./admin-DiPFJU_L.js");
var Route$2 = createFileRoute("/admin")({
	head: () => ({ meta: [{ title: "Admin Stewardship — Turath" }, {
		name: "description",
		content: "Oversee sellers, books, users, categories, support tickets and orders on Turath."
	}] }),
	component: lazyRouteComponent($$splitComponentImporter$2, "component")
});
//#endregion
//#region src/routes/seller.tsx
var $$splitComponentImporter$1 = () => import("./seller-ZvZYaYjY.js");
...
var $$splitComponentImporter = () => import("./shop-Be6iJy2-.js");
