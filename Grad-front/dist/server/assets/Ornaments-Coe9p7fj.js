...
function TurathProvider({ children }) {
	...
	const value = useMemo(() => {
		...
		const activeUser = authUserId ? users.find((u) => u.id === authUserId) ?? users.find((u) => storedEmail && u.email.toLowerCase() === storedEmail.toLowerCase()) ?? {
			id: authUserId,
		...
		return {
			...
			signIn: (email) => {
				...
				const updatedUsers = user ? users.map((existing) => existing.id === user.id ? {
					...existing,
					id: authenticatedId,
					role: realRole
				} : existing) : [{
...
//#endregion
export { TAX_RATE as a, egp as c, apiFetch as d, currentUserId as f, cn as i, roleLabels as l, LeafSprig as n, TurathProvider as o, Badge as r, avgRating as s, BranchDivider as t, useTurath as u };
